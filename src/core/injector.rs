...
pub fn inject_dll(process_id: u32, dll_path: &str) -> Result<()> {
    ...
    if dll_path.is_empty() {
        bail!("DLL path is empty");
    }
    if !std::path::Path::new(dll_path).exists() {
        bail!("DLL file does not exist: {}", dll_path);
    }
    ...
    unsafe {
        ...
        if process_handle.is_invalid() {
            bail!("Failed to open target process. Process may not exist or insufficient permissions.");
        }
        ...
        if bytes_written != dll_path_bytes.len() {
            bail!("Failed to write complete DLL path to target process memory.");
        }
        ...
        if thread_handle.is_invalid() {
            bail!("Failed to create remote thread in target process.");
        }
